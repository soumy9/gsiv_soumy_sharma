...
const filter='include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc';
...
export const getMoviesList = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    ...
    const moviesList = await (
      await fetch(`${api}/discover/movie?api_key=${apiKey}&${filter}&page=${page}`)
    ).json();
